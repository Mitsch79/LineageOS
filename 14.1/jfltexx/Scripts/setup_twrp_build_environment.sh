...
clear
if [ -z -z "$(grep -E 'LineageOS/android_frameworks_base|LineageOS/android_external_junit' .repo/local_manifests/roomservice.xml)" ]; then
	...
	echo "Zusätzliche Repos bereits in roomservice.xml vorhanden. Weiter"
fi;
...
if [ ! -e device/samsung/jfltexx/twrp.fstab ]; then
	echo "Bitte kopiere die folgenden Zeilen in die Zwischenablage"
	echo -e "${RED}"
...
if [ -z "$(cat device/samsung/jfltexx/BoardConfig.mk | grep 'PRODUCT_COPY_FILES += device/samsung/jfltexx/twrp.fstab:recovery/root/etc/twrp.fstab')" ]; then
	echo "Bitte kopiere die folgende Zeile in die Zwischenablage"
	echo -e "${RED}"
